...
if (isset($_SESSION['name'])) {
  $colname_name = $_SESSION['name'];
}
mysql_select_db($database_saap, $saap);
$query_name = sprintf("SELECT name FROM users WHERE name = %s", GetSQLValueString($colname_name, "text"));
$name = mysql_query($query_name, $saap) or die(mysql_error());
...
?>
        ...
        <div class="form-group">
        Nombre del empleado           
       <input name="nombre" type="text" class="form-control" placeholder="Ingrese su usuario"  />
          </select>
        </div>
    <hr />
